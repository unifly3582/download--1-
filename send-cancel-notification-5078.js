// Manually send cancellation notification for order 5078
...
async function sendCancellationNotification() {
  console.log('📤 Sending cancellation notification for order 5078\n');

  try {
    // Get order details
    const orderDoc = await db.collection('orders').doc('5078').get();
    
    if (!orderDoc.exists) {
      console.error('❌ Order 5078 not found');
      return;
    }
    ...
    if (response.ok) {
      ...
      await db.collection('orders').doc('5078').update({
